<x-app-layout>
    ...
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Editar Tarefa: ') }} <span class="italic">{{ $tarefa->titulo }}</span>
        </h2>
    </x-slot>
        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 md:p-8 bg-white border-b border-gray-200">
                    ...
                    <form method="POST" action="{{ route('task.update', $tarefa->id) }}">
                        @csrf
                        @method('PUT') 
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div class="mb-6">
                                <label for="tempoLimite" class="block text-sm font-medium text-gray-700 mb-1">Data Limite (Opcional)</label>
                                <input type="datetime-local" name="tempoLimite" id="tempoLimite"
                                       value="{{ old('tempoLimite', $tarefa->tempoLimite ? \Carbon\Carbon::parse($tarefa->tempoLimite)->format('Y-m-d\TH:i') : '') }}"
                                       class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                            </div>
                        </div>
